...
bool Temperature::thresholdCompare(unsigned short tempValue) {
  bool fault = false;
  if(tempValue > (Config::State().tempThresholdValue / 2)) {      //Compare readout temperature value with temperature threshold value set by rotary encoder. Temp threshold value is divided by 2 to give correct temperature value.
